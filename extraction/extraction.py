...
def clean_df(df):
    ...
    for col in boolean_cols:
        if col in df.columns:
            ...
            logger.debug(
                f"Columna '{col}' convertida a booleano. Nulos: {before_nulls} -> {after_nulls}"
            )
    ...
    if "job_posted_date" in df.columns:
        ...
        logger.info(
            f"Columna 'job_posted_date' convertida. Fechas inválidas: {invalid_dates}"
        )
    ...
    for col in numeric_cols:
        if col in df.columns:
            ...
            logger.debug(
                f"Columna '{col}' convertida a numérico. Valores inválidos: {invalid_nums}"
            )
...
def parse_type_skills_string(type_skills_str: str) -> Optional[Dict[str, List[str]]]:
    ...
    try:
        ...
        logger.warning(
            f"Error parseando type_skills: {type_skills_str[:50]}... - {str(e)}"
        )
        return None
...
def skills_parser(df):
    ...
    logger.info(
        f"Usando {n_workers} workers para procesamiento paralelo (de {n_cores} cores disponibles)"
    )
    ...
    if "job_skills" in df.columns:
        ...
        if len(df) > 10000 and n_workers > 1:
            ...
            with ThreadPoolExecutor(max_workers=n_workers) as executor:
                df["job_skills"] = list(
                    executor.map(parse_skills_string, df["job_skills"])
                )
        else:
        ...
        logger.info(
            f"job_skills procesados: {skills_count:,} registros válidos (antes: {before_count:,})"
        )
    else:
    ...
    if "job_type_skills" in df.columns:
        ...
        if len(df) > 10000 and n_workers > 1:
            ...
            with ThreadPoolExecutor(max_workers=n_workers) as executor:
                df["job_type_skills"] = list(
                    executor.map(parse_type_skills_string, df["job_type_skills"])
                )
        else:
            df["job_type_skills"] = df["job_type_skills"].apply(
                parse_type_skills_string
            )

        type_skills_count = df["job_type_skills"].notna().sum()
        logger.info(
            f"job_type_skills procesados: {type_skills_count:,} registros válidos (antes: {before_count:,})"
        )
    else:
...
def load_optimized_fast(path: str) -> bool:
    ...
    try:
        ...
        logger.info(
            f"CSV cargado en memoria: {len(df):,} registros en {load_time:.2f}s"
        )
        ...
        logger.info(
            f"Sistema: {specs['cpu_cores']} cores, {specs['available_memory_gb']:.1f}GB RAM disponible"
        )
        ...
        logger.info(
            f"Inserción completada: {len(df):,} registros en {insert_time:.2f}s ({records_per_sec:.0f} rec/s)"
        )
        ...
        try:
            ...
            if total_count != len(df):
                logger.warning(
                    f"Discrepancia: esperados {len(df):,}, encontrados {total_count:,}"
                )
...
def load_with_copy(path: str) -> bool:
    ...
    try:
        ...
        df = pd.read_csv(path, engine="c", low_memory=False)
        logger.info(
            f"CSV cargado: {len(df):,} registros en {time.time() - load_start:.2f}s"
        )
        ...
        records_per_sec = len(df) / insert_time if insert_time > 0 else 0
        logger.info(
            f"COPY completado: {len(df):,} registros en {insert_time:.2f}s ({records_per_sec:.0f} rec/s)"
        )
...
def execute_extraction(csv_path, use_copy=False):
    ...
    logger.info(
        "  • Método: %s", "PostgreSQL COPY" if use_copy else "Inserción paralela"
    )
    ...
    try:
        ...
        if success:
            ...
            logger.info("╔" + "═" * 58 + "╗")
            logger.info(
                "║" + " " * 10 + "✓ PIPELINE COMPLETADO EXITOSAMENTE" + " " * 13 + "║"
            )
            logger.info("╚" + "═" * 58 + "╝")
            logger.info("Tiempo total de ejecución: %.2f segundos", duration)
            logger.info(
                "Velocidad promedio: %.0f registros/segundo",
                785640 / duration if duration > 0 else 0,
            )
            logger.info(
                "Fin de ejecución: %s", datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            )
        else:
            logger.error("")
            logger.error("╔" + "═" * 58 + "╗")
